...
class MixerEngine:
    ...
    def run_async_task(self, inputs: Dict[str, Any]) -> np.ndarray:
        """
        ...
        Args:
            inputs: Dictionary containing:
                ...
                - 'weights1': Dict[int, float] (Mag or Real weights)
                - 'weights2': Dict[int, float] (Phase or Imag weights)
                - 'images': List[ImageModel]
    ...
    def mix_images_mag_phase(
        ...
        # Safety check for division by zero
        if total_phase_weight == 0:
            total_phase_weight = 1.0

        for idx, w in phase_sources.items():
            if idx < len(images) and images[idx] is not None:
                # CRITICAL FIX: Use get_data()
                mixed_phase += images[idx].get_data('phase') * (w / total_phase_weight)

        # Apply mask to magnitude if provided
        if mask is not None:
            if mask.shape != shape:
                # Keep original error logic
                raise ValueError("Mask shape does not match image shape")
            mixed_magnitude *= mask
            # TODO
            # mixed_phase*=mask
            # Note: Masking magnitude is usually sufficient for region mixing.
